  ...
  <div className="bg-white rounded-xl p-6 border border-slate-100 shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col justify-between h-full group">
    <div className="flex justify-between items-start mb-4">
      <div className={`p-2 rounded-lg ${accentColor} bg-opacity-10 text-opacity-100`}>
        {React.cloneElement(icon as React.ReactElement, { className: `w-5 h-5 ${accentColor.replace('bg-', 'text-')}` })}
      </div>
...
export const MissionGrid: React.FC = () => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 auto-rows-fr">
      ...
      {/* Card A: The Mission (Spans 2 cols) */}
      <div className="col-span-1 md:col-span-2 bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl p-8 shadow-md text-white flex flex-col justify-center relative overflow-hidden group">
        ...
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-blue-500 rounded-full mix-blend-overlay filter blur-3xl opacity-10 group-hover:opacity-20 transition-opacity duration-500"></div>

        <div className="relative z-10">
          <div className="inline-flex items-center px-3 py-1 rounded-full bg-white/10 border border-white/20 text-xs font-medium text-emerald-300 mb-4">
            Mission Control
          </div>
          <h2 className="text-2xl md:text-3xl font-bold mb-3 leading-tight">
            Tracking the Real-World Impact of <span className="text-emerald-400">Gies Business Research</span>
          </h2>
          <p className="text-slate-300 text-base md:text-lg mb-6 max-w-lg leading-relaxed">
